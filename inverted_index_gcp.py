...
BUCKET = CLIENT.bucket('318457645')
...
#         return b''.join(b)
    def read(self, locs, n_bytes,base_dir):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = BUCKET.get_blob(f'postings_gcp{base_dir}/{f_name}')
...
class InvertedIndex:
    ...
    @staticmethod
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name,dir_name):
        with open(f"{bucket_id}_posting_locs.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"postings_gcp{dir_name}/{bucket_id}_posting_locs.pickle")
